...
namespace MXPlatformAPI.Controllers
{
    public class InstitutionController : Controller
    {
        ...
        public async Task<ResponseMessage> ListInstitutions()
        {
            ...
            InstitutionRepository _institutionRepository = new();
            try
            {
                var result = await _institutionRepository.ListInstitutions(baseUrl, basicAuth);
                if (result.Status)
                {
        ...
        public async Task<ResponseMessage> GetInstitutionCredential(string instituteCode)
        {
            ...
            try
            {
                ...
                var result = await _institutionRepository.GetInstituteCredential(instituteCode, baseUrl, basicAuth);
                if (result.Status)
                {
                    _response.Data = result.Data;
                    _response.Status = true;
