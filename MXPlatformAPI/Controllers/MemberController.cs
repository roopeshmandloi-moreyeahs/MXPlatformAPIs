    ...
    public class MemberController : Controller
    {
        ...
        public async Task<ResponseMessage> CreateMember([FromBody] CreateMemberRoot member, string UserGuid)
        {
            ...
            try
            {
                ...
                #endregion
                var result = await MemberRepository.CreateMember(member, UserGuid, baseUrl, basicAuth);
                if (result.Status)
                {
        ...
        public async Task<ResponseMessage> ListMembers(string UserGuid)
        {
            ...
            try
            {
                var result = await MemberRepository.ListMembers(UserGuid, baseUrl, basicAuth);
                if (result.Status)
                {
                    _response.Data = result.Data;
                    _response.Status = true;
        ...
        public async Task<ResponseMessage> CheckMemberStatus(string UserGuid, string MemberGuid)
        {
            ...
            try
            {
                var result = await MemberRepository.CheckMemberStatus(UserGuid, MemberGuid, baseUrl, basicAuth);
                if (result.Status)
                {
                    _response.Data = result.Data;
                    _response.Status = true;
