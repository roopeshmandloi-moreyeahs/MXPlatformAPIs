...
namespace MXPlatformAPI.Controllers
{
    public class MemberController : Controller
    {
        ...
        public async Task<ResponseMessage> CreateMember([FromBody] CreateMemberRoot member, string userGuid)
        {
            ...
            Data data = new();
            MemberRepository _memberRepository = new();
            ...
            try
            {
                ...
                #endregion
                var result = await _memberRepository.CreateMember(member, userGuid, baseUrl, basicAuth);
                if (result.Status)
                {
        ...
        public async Task<ResponseMessage> ListMembers(string userGuid)
        {
            ...
            try
            {
                var result = await _memberRepository.ListMembers(userGuid, baseUrl, basicAuth);
                if (result.Status)
                {
                    _response.Data = result.Data;
                    _response.Status = true;
        ...
        public async Task<ResponseMessage> CheckMemberStatus(string userGuid, string memberGuid)
        {
            ...
            try
            {
                var result = await _memberRepository.CheckMemberStatus(userGuid, memberGuid, baseUrl, basicAuth);
                if (result.Status)
                {
                    _response.Data = result.Data;
                    _response.Status = true;
